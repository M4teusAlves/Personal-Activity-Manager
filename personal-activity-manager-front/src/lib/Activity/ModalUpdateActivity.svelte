<script>
    ...
    async function fetchUpdateActivity() {
      loadingData = true;
      try {
        const response = await fetch(`http://localhost:3001/activity/${id}`, {
          method: "PUT",
...
{#if show}
    <div>
        <form on:submit={fetchUpdateActivity}>
            <header>
                <h2>Atualização de Atividade</h2>
                <button on:click={setShow()}><X/></button>
            </header>
            
            <label for="firstName">
                Activity:
                <input type="text" bind:value={activity.description} required maxlength="20">
            </label>
